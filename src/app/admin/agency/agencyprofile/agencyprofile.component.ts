...
export class AgencyprofileComponent implements OnInit {
  ...
  addUser(
    ...
    // tslint:disable-next-line:max-line-length
    if (
      this._mysqlService.addMysqlUserDatas(
        USER_ID,
        COMPANYCODE,
        EMAIL,
        REG_DATE,
  ...
  // tslint:disable-next-line:max-line-length
  UpdateProfile(STREET: string, CITY: string, POSTAL_CODE: number, COUNTRY: string, FULL_NAME_LEGACY: string, VAT_NUMBER: string, IATA_NUMBER: string, TITLE: string, FIRST_NAME: string, MIDDLE_NAME: string, LAST_NAME: string, PHONE1: number, PHONE2: number, FAX: number, WEBSITE: string, EMAIL: string, LANGUAGE: string, PRICE_FORMAT: string, DATE_FORMAT: string, TIME_ZONE: string, TIME_FORMAT: string, LOGO: string) {
if (
      // tslint:disable-next-line:max-line-length
      this._mysqlService.UpdateMysqlUserDatas(STREET, CITY, POSTAL_CODE, COUNTRY, FULL_NAME_LEGACY, VAT_NUMBER, IATA_NUMBER, TITLE, FIRST_NAME, MIDDLE_NAME, LAST_NAME, PHONE1, PHONE2, FAX, WEBSITE , EMAIL, LANGUAGE, PRICE_FORMAT, DATE_FORMAT, TIME_ZONE, TIME_FORMAT, LOGO)
    ) {
